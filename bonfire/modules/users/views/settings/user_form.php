...
?>
    ...
    <fieldset>
        ...
        <div class="control-group col-sm-12">
            ...
            <div class="controls">
                <select name="role_id" id="role_id" class="select2 <?php echo $controlClass; ?> form-control">
                    <?php
                    if (isset($roles) && is_array($roles) && count($roles)) :
                        foreach ($roles as $role) :
                            if (has_permission('Permissions.' . ucfirst(trim($role->role_name)) . '.Manage')) :
                                // check if it should be the default
                                $default_role = false;
                                if ((isset($user) && $user->role_id == $role->role_id)
                                    || ( ! isset($user) && $role->is_default == 1)
                                   ) {
                                    $default_role = true;
                                }
                    ?>
